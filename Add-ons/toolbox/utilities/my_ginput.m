function [out1,out2,out3] = my_ginput(arg1, cursor_color)
...
% From https://fr.mathworks.com/matlabcentral/profile/authors/1343420-image-analyst
% Modified: 04/27/2018 (mm/dd/yyyy) Tunable color for the cursor.
%   (Default = white)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig, cursor_color);
...
function initialState = setupFcn(fig, cursor_color)
...
initialState.CrossHair = createCrossHair(fig, cursor_color);
...
function crossHair = createCrossHair(fig, cursor_color)
...
for k = 1:4
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', cursor_color, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
end
end
